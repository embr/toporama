...
(function (root, factory) {
  ...
  function makeBottomHull(top, minTopThickness, minSideThickness, minZval, wallMaskIn) {
    ...
    var wallMask = wallMaskIn || getWallVertexMask(top, minSideThickness);
    ...
    if (minZ === null || minZ === undefined) {
      ...
      // The offset hull thins on slopes: a tilted normal drops less than
      // minTopThickness in z, so when the LOWEST top point sits on a slope
      // (rather than on the flat rim) the base plane lands above it and the
      // shell is thinner than the material minimum right there. Never let
      // the base sit less than a full thickness below the lowest top point.
      // This only ever adds material, and leaves models whose minimum is
      // already on a vertical wall vertex unchanged.
      var minTopZ = Infinity;
      for (i = 0; i < V; i++)
        if (top.vertices[i * 3 + 2] < minTopZ) minTopZ = top.vertices[i * 3 + 2];
      if (minTopZ - minTopThickness < minZ) minZ = minTopZ - minTopThickness;
    }
  ...
  function makeBottom(top, minTopThickness, minSideThickness, minZval, wallMaskIn) {
    var hull = makeBottomHull(top, minTopThickness, minSideThickness, minZval, wallMaskIn);
    var V = top.numVertices();
  ...
  function pinBaseRequirement(top, m, n, holes, xyScale) {
    ...
    holes.locations.forEach(function (loc, li) {
      // holes.local carries pin positions already expressed in the
      // shape's local frame (mercator metres, pre-rescale); rotated
      // selections need that, since absolute mercator no longer matches
      // the axes the mesh is built on.
      var p = holes.local ? holes.local[li] : project(loc[0], loc[1]);
